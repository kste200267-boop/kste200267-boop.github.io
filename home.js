...
var PageHome=(function(){
  ...
  function loadCalendarEvents(targetYmd){
    ...
    // localStorage 캐시에서 먼저
    var raw=null;
    try{raw=localStorage.getItem('weekly-public-events-v2')}catch(e){}
    var events=null;
    if(raw){
      try{
        var parsed=JSON.parse(raw);
        if(parsed&&parsed.events) events=parsed.events;
      }catch(e){}
    }
    ...
    if(!events){
      try{
        var fb=firebase&&firebase.apps&&firebase.apps.length?firebase.firestore():null;
        if(fb){
          fb.collection('portal').doc('weekly-events').get().then(function(doc){
            if(doc.exists&&doc.data().events){
              renderWeeklyOnHome(doc.data().events,targetYmd);
            }else{
              var el2=document.getElementById('homeWeekly');
              if(el2) el2.innerHTML='<span style="color:var(--tx3);font-size:.82em">구글캘린더 연동 필요<br>주간업무 탭에서 관리자 로그인</span>';
            }
          }).catch(function(){
            var el2=document.getElementById('homeWeekly');
            if(el2) el2.innerHTML='<span style="color:var(--tx3);font-size:.82em">불러오기 실패</span>';
          });
          return;
        }
      }catch(e){}
      el.innerHTML='<span style="color:var(--tx3);font-size:.82em">구글캘린더 연동 필요<br>주간업무 탭에서 로그인</span>';
      return;
    }
