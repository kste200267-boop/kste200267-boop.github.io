// pages/my-timetable.js — 내 시간표 (방과후 요일별 교시 다름: 월화=8·9·10교시, 수목금=7·8·9교시)
var PageMy=(function(){
  ...
  // 구버전 데이터(p8,p9) → 신버전(slots) 마이그레이션
  function migrateAfterItem(item){
    if(item.slots) return item; // 이미 신버전
    var slots=[];
  ...
  function render(){
    ...
    for(var p=0;p<maxP;p++){
      ...
      for(var di=0;di<5;di++){
        var d=DAYS[di];
        if(p>=DP[d]){h+='<td class="emp" style="background:var(--bg3)">—</td>';continue}
    ...
    sortedPeriods.forEach(function(period){
      // 이 교시가 있는 요일이 하나라도 있으면 행 표시
      var hasAny=DAYS.some(function(d){
        return (AFTER_DEF[d]||[]).some(function(def){return def.p===period});
      });
      if(!hasAny) return;

      h+='<tr style="background:#fdf4ff">';
      // 교시 라벨
      var defLabel=period+'교시(방과후)';
      // 시간은 월요일 기준으로 표시
      var timeStr='';
      ...
      h+='<th style="background:#e1bee7;font-size:.76em;padding:5px 4px;white-space:nowrap">'+defLabel+'<br><span style="font-weight:400">'+timeStr+'</span></th>';

      for(var di=0;di<5;di++){
        var d=DAYS[di];
        // 이 요일에 이 교시가 방과후 교시인지 확인
    ...
    h+='<p style="color:var(--tx2);font-size:.82em;margin-bottom:10px">';
    h+='월·화: 8·9·10교시 / 수·목·금: 7·8·9교시 &nbsp;|&nbsp; 기간 설정 가능</p>';
  ...
  function editAfterSlot(day,period){
    ...
    val=val.trim();
    if(!found){
      found={day:day,slots:[],from:'',to:''};
      data.push(found);
    }
    // slots 업데이트
    var slotIdx=-1;
    for(var j=0;j<found.slots.length;j++){
      if(found.slots[j].period===period){slotIdx=j;break}
    }
    if(val){
